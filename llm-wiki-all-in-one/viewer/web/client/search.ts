...
export function installSearchUI(options: InstallSearchOptions): void {
  ...
  const renderResults = (): void => {
    ...
    resultsEl.innerHTML = results
      .map((result, index) => renderResult(result, index === activeIndex, result.path === options.getCurrentPath()))
      .join("");
  ...
  const moveActive = (delta: number): void => {
    ...
    renderResults();
    resultsEl.querySelector<HTMLButtonElement>(".search-result.active")?.scrollIntoView({ block: "nearest" });
  };
